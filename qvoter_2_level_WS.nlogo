...
Here, we introduce an extension of the q-voter model with independence, which takes into account that there may be a discrepancy between agents' private opinion (true beliefs) and public opinion (the expressed opinion or behavior). We assume that agents can only see each other's public opinions. If an agent has different opinions on these two levels (private and public), he is in what is called cognitive dissonance. Again, there are two possible responses:conformity and independence, but they are different at the private and public levels. The model we present here is the modification of the model introduced in [4]. This modification of the model is to account for the fact that people usually try to reduce cognitive dissonance[5].


In the case of public opinion, independence means that the agent expresses her true (private) opinion, and thus her public opinion is equal to her private one. Conformity, on the other hand, corresponds to the situation where the agent adapts its behavior to other agents. We randomly choose q neighbors (q-panel) that will be the source of social influence. If an agent is initially in internal harmony, i.e. has the same private and public opinion, it will be less susceptible to social pressure. In this case, unanimity of the q-panel is required. In other words, when q voters express the same opinion, the considered agent changes her public opinion to be the same as theirs, even though this may lead to dissonance. This type of conformity is called compliance in social psychology. On the other hand, if the agent is initially in internal conflict, she will be more willing to change. Thus, it is enough for at least one person to express an opinion that is consistent with the private beliefs of the considered agent in order to encourage her to express them. Thus, we replace the public opinion of the agent with the private one if among the selected q-voters there is at least one who shares the private opinion of the public agent. This type of influence is known in psychology as disinhibitory contagion.
...
1) Randomly choose one of the N agents to reconsider her opinion, we will call her the "target", and update the counter: time = time + 1

2) ACT 
With probability p, the target behaves independently, so she sets public opinion equal to private opinion. 
With complementary probability (1-p), she is susceptible to the influence of her neighbors (agents directly connected to the target). She chooses q neighbors to form the q-panel 
 - If the target's public opinion is the same as her private opinion: she checks if all agents from the q-panel have the same opinion - if so, she sets the public opinion to the same as her own (compliance) 
 - Otherwise, the target checks if at least one agent from the q-panel has the same opinion as her own, and if so, she sets the public opinion to the same as her private opinion (disinhibitory contagion)

3)THINK
With probability p the target behaves independently - with probability 1/2 the private opinion changes to the opposite. 
With complementary probability (1-p) she randomly selects q panels from her neighbors and checks if they all have the same public opinion as her public one - if so, she sets her private opinion to the same public one
...
0) "setup"- to set all values of parameters. Note that this time only public opion of agents are visible in order to present, what agents really know about each other. Herein some agents may initially be in dissonance to ease observation of possible scenarios.
1) "target" - an agent, which opinion will be updated is selected. Thick black lines indicate the target's neighbors. Now its private opinion is shown
